...
- `FIXNOW_MAP_TILE_URL` — optional OSM-compatible HTTPS tile template containing
  `{z}`, `{x}` and `{y}`. When omitted, FixNow uses the official interactive
  OpenStreetMap URL. Do not add bulk download, prefetch, or offline-map features.
- `FIXNOW_MAP_TILE_USER_AGENT` — use the stable Android application ID so native
  tile requests identify FixNow. The app shows visible OpenStreetMap attribution.
